...
create or replace procedure test_user_addresses( in muid integer unsigned,
                                                 in action_mode integer )
        modifies sql data
        sql security invoker
begin   /* {{ */
    ...
    select vid from users where uid = muid into xvid ;

    select ifnull(max(delta)+1,0) from field_data_field_addresses where entity_id = muid into xseq ;
    ...
    select 'Step 1' as `Action`, 'field_data_field_addresses' as `Table`,
             ...
             muid as `entity_id`, xvid as `revision_id`, @deflang as
    ...
    values ( 'user',      'user', false,   muid,      xvid,        @deflang, xseq,  xiid,                  xrid) ;
    insert into field_revision_field_addresses select * from field_data_field_addresses where entity_id = muid and revision_id = xvid and delta = xseq ;
...
begin   /* {{ */
    ...
    declare user_address_slots cursor for
        select address_uid,              /* int(11) unsigned not null default 0     */
               ...
               address_rev,              /* int(11) unsigned          default NULL  */
    ...
    if purge_first then
        call purge_user_addresses( muid, inner_action_mode ) ;
    end if ;
...
if false then
    /* These have already been inserted by add_collection_item */
        insert into field_collection_item (
                                                item_id,
                                                revision_id,
                                                field_name,
                                                archived,
                                                uuid )
        select xiid                          as item_id,                   /* == field_addresses_value       */
               xrid                          as revision_id,               /* == field_addresses_revision_id */
               'field_addresses'             as field_name,
               false                         as archived,
               uuid()                        as uuid
